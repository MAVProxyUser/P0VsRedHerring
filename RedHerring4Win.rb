...
if File.readlines("C:/windows/system32/drivers/etc/hosts").grep(/flysafe\.aasky\.net/).size > 0
  print "Flysafe redirection already in hosts file\n"
else
  print "You need to add 127.0.0.1 flysafe.aasky.net to /etc/hosts\n"
  #File.open("C:/windows/system32/drivers/etc/hosts", 'a') {|f| f.write("\n127.0.0.1 flysafe.aasky.net\n") }
  exit
end

if File.readlines("C:/windows/system32/drivers/etc/hosts").grep(/swsf\.djicorp\.com/).size > 0
  print "Swsf DJICorp redirection already in hosts file\n"
else
  print "You need to add 127.0.0.1 swsf.djicorp.com to /etc/hosts"
...
pid = spawn("C:/Program Files (x86)/DJI Product/DJI Assistant 2__V1.1.2.573/Assistant --test_server --factory")
Process.detach(pid)
